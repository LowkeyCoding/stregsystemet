...
def quicksale(request, room, member, bought_ids):
    ...
    try:
        ...
        return render(
            request,
            "stregsystem/error_productdoesntexist.html",
            {"failedProduct": i, "room": room},
        )
...
# [EDIT]
def menu_userinfo(request, room_id, username):
...
# [EDIT]
def menu_userpay(request, room_id, username):
    room = Room.objects.get(pk=room_id)
    member = Member.objects.get(username=username, active=True)
...
# [EDIT]
def menu_sale(request, room_id, username, product_id=None):
    room = Room.objects.get(pk=room_id)
    news = __get_news()
    member = Member.objects.get(username=username, active=True)
...
def mobilepaytool(request):
    paytool_form_set = modelformset_factory(
        ...
        extra=0,
        fields=("timestamp", "amount", "member", "comment", "status"),
